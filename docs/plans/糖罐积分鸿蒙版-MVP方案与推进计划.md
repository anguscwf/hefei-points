...
3. **首发先核心后增强：** 历史 `0.1.0 (10000)` 仅为阶段 0 诊断包；当前跟踪的 `0.2.0 (20000)` 仍只是未连接外部服务、未发布且不完整的 S10 本地安全壳，S11～S13 也只补齐临时 synthetic 客户端工作区、服务端配置/预检和离线数据根准备工具，均不能直接公开上架。完成“监护人授权与配对 → 查看积分 → 申报任务 → 家长审批 → 查看结果 → 解绑/删除”的薄 MVP 及全部发布硬门后，才能生成正式首发候选版。
...
1. 保持 HarmonyOS `NETWORK_ENABLED=false`、跟踪小程序 develop/trial 零联网配置、`.invalid` 源和全部儿童生产功能门关闭；S9 loopback 全链、S10 工程安全壳、S11 临时小程序生成器、S12 配置/预检与 S13 数据根工具只作为本地基线，不生成签名包、不连接外部业务服务或生产。
2. 设计并实现 S14“合成环境初始管理员与法律证据安全 bootstrap”。它只在合成配置、临时目录和临时 SQLite 上建立一次性、可审计、幂等、失败完整回滚的 bootstrap 契约，不实际部署，不硬编码密码/密钥/真实身份，不写入正式法律文本或生产数据，也不开放儿童 production 门。
3. S14 通过后，由受权操作员按 S13 操作手册在仓库和生产数据根之外准备专用 synthetic 数据根，落实专用 OS 账号、ACL/所有权、磁盘/备份边界和独立密钥；工具输出只证明本地结构，实际物理边界仍须人工核对。
4. 外部验证独立 synthetic AppID provisioning、开发者权限、AppSecret 独立性、request/business domain、DNS/TLS、基础设施、法律记录和数据库内容隔离，并确认 DevTools 私有设置未关闭域名/TLS 校验。候选配置先运行 S12 已提交离线 verifier；只有其无秘密证据和全部外部核对均通过，才另行审批一次受控 synthetic 部署。客户端只使用 S9/S11 受控系统临时配置，禁止运行时改源或访问生产 host。
