...
3. **首发先核心后增强：** 历史 `0.1.0 (10000)` 仅为阶段 0 诊断包；当前跟踪的 `0.2.0 (20000)` 仍只是未连接外部服务、未发布且不完整的 S9 合成就绪切片，都不能直接公开上架。完成“监护人授权与配对 → 查看积分 → 申报任务 → 家长审批 → 查看结果 → 解绑/删除”的薄 MVP 及全部发布硬门后，才能生成正式首发候选版。
...
1. 保持 HarmonyOS `NETWORK_ENABLED=false`、`.invalid` 源和全部儿童生产功能门关闭；S9 只作为本地合成端到端就绪基线，不生成签名包、不连接外部业务服务或生产。
...
5. 实现儿童易懂版隐私摘要与设置，明确未决申报文字的保留期限和监护人授权安全放弃流程；设备作用域单条详情和写操作对账完成前，不开放申请补充、取消或重新提交。
