...
> 文档版本：v0.8（S11 小程序受控 synthetic 工作区记录）
...
3. **首发先核心后增强：** 历史 `0.1.0 (10000)` 仅为阶段 0 诊断包；当前跟踪的 `0.2.0 (20000)` 仍只是未连接外部服务、未发布且不完整的 S10 本地安全壳，S11 也只补齐家长小程序的临时 synthetic 工程准备，二者都不能直接公开上架。完成“监护人授权与配对 → 查看积分 → 申报任务 → 家长审批 → 查看结果 → 解绑/删除”的薄 MVP 及全部发布硬门后，才能生成正式首发候选版。
...
1. 保持 HarmonyOS `NETWORK_ENABLED=false`、跟踪小程序 develop/trial 零联网配置、`.invalid` 源和全部儿童生产功能门关闭；S9 loopback 全链、S10 工程安全壳与 S11 临时小程序生成器只作为本地基线，不生成签名包、不连接外部业务服务或生产。
2. 建立获批、物理、域名、凭据和数据均独立的合成非生产 API，只通过 S9/S11 受控系统临时编译配置供小程序开发/体验环境和 HarmonyOS develop profile 使用；禁止运行时改源或预览访问生产 host。先外部验证 synthetic AppID provisioning、开发者权限、request/business domain、DNS/TLS 和基础设施隔离，并确认 DevTools 私有设置未关闭域名/TLS 校验。
3. 在 HarmonyOS 模拟器或成人受控 API 20+ 设备完成“授权 → 配对 → 查积分/规则 → 申报 → 家长审批 → 查看结果 → Refresh → 撤销”端到端 smoke，并验证 HUKS、AssetStore、前后台、重启、结果未知恢复和清数据行为。
4. 在微信开发者工具和成人受控设备完成家长端同链路 smoke，交付安全文件导出/披露回执；全部证据只使用合成家庭且不记录凭据或儿童个人信息。
5. 以 S10 工程安全壳为边界，由合规工作流定稿儿童易懂版正式摘要、完整法律文本、处理者联系方式、客服/投诉和真实行权入口；不得把工程说明当作同意或法律页面。并明确未决申报文字的保留期限和监护人授权安全放弃流程；设备作用域单条详情和写操作对账完成前，不开放申请补充、取消或重新提交。
6. 完成正式《隐私政策》《儿童个人信息保护规则》《儿童用户协议》、PIPIA、存量儿童数据逐类整改、留存/删除规则、受托方约束、儿童信息保护负责人和安全事件流程。
7. 完成生产密钥托管与轮换、加密备份/恢复演练、迁移清单门、监控、回滚、正式法律页面及其重定向/CSP/业务域名验证。
8. 准备 APP 备案、内容分级、隐私标签、发布素材、客服、审核测试家庭和可复现操作说明；使用全新成人测试账号和合成孩子完成从零成人预验收。
9. 仅在上述工程、合规和发布硬门全部通过后，使用获批 RELEASE 证书/Profile 构建并验证 `0.2.0 (20000)` 正式候选包，再提交首次公开发布。
10. 正式上架后，未成年人账号保持未成年人模式和远程守护，从 AppGallery 安装并验收核心闭环；不使用侧载、切换账号、关闭未成年人模式或孩子设备开发者模式。
11. 核心试点稳定后，重新确认届时生效的上学/假期用机规则，再以 `0.3.0 (30000)` 为目标实现积分冻结和人工用机兑换；`0.2.1 (20001)` 仅保留给首发修复。
