...
> 文档版本：v0.5（S8 HarmonyOS 积分申报纵向切片记录）
...
3. **首发先核心后增强：** 历史 `0.1.0 (10000)` 仅为阶段 0 诊断包；当前跟踪的 `0.2.0 (20000)` 仍只是未联网、未发布且不完整的 S8 安全切片，都不能直接公开上架。完成“监护人授权与配对 → 查看积分 → 申报任务 → 家长审批 → 查看结果 → 解绑/删除”的薄 MVP 及全部发布硬门后，才能生成正式首发候选版。
...
1. 保持 HarmonyOS `NETWORK_ENABLED=false`、`.invalid` 源和全部儿童生产功能门关闭；S8 只作为本地合成验证基线，不生成签名包或连接生产。
