...
3. **首发先核心后增强：** 历史 `0.1.0 (10000)` 仅为阶段 0 诊断包；当前跟踪的 `0.2.0 (20000)` 仍只是未连接外部服务、未发布且不完整的 S10 本地安全壳切片，不能直接公开上架。完成“监护人授权与配对 → 查看积分 → 申报任务 → 家长审批 → 查看结果 → 解绑/删除”的薄 MVP 及全部发布硬门后，才能生成正式首发候选版。
...
1. 保持 HarmonyOS `NETWORK_ENABLED=false`、`.invalid` 源和全部儿童生产功能门关闭；S9 loopback 全链与 S10 工程安全壳只作为本地基线，不生成签名包、不连接外部业务服务或生产。
2. 建立获批、物理与凭据均独立的合成非生产 API，只通过受控编译时配置供小程序开发/体验环境和 HarmonyOS develop profile 使用；禁止运行时改源或预览访问生产 host。
