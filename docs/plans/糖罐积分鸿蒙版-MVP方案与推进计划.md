...
> 文档版本：v0.4（S7 HarmonyOS 安全纵向切片记录）
> 初版日期：2026-08-09；修订日期：2026-08-25
...
3. **首发先核心后增强：** 历史 `0.1.0 (10000)` 仅为阶段 0 诊断包；当前跟踪的 `0.2.0 (20000)` 仍只是未联网、未发布且不完整的 S7 安全切片，都不能直接公开上架。完成“监护人授权与配对 → 查看积分 → 申报任务 → 家长审批 → 查看结果 → 解绑/删除”的薄 MVP 及全部发布硬门后，才能生成正式首发候选版。
