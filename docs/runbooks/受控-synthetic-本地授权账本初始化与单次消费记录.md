...
外部处置至少应保存脱敏错误码、候选 commit、Node.js 版本和时间线；不得保存输入、签名正文、真实资源身份、路径或秘密。S18 只能把精确历史 receipt 准备成本地未提交意图；是否恢复备份、重建 ledger 或轮换策略必须由 S19 的获批权威流程决定，S17/S18 都不自动删除或接管未知状态。
