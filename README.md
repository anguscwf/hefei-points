...
本仓库是项目源码与核心工程文档的唯一真源。当前基线包含 Node.js 后端、微信小程序家长端、HarmonyOS 孩子端安全纵向切片，以及 S9～S16 的受控 synthetic 本地准备、候选证据封装与外部签名束离线验证能力。
...
- 阶段 1 已完成 S0～S16 的安全、授权、配对、本人视图、积分申报、数据行权、两端客户端和 synthetic 运行前置切片。S13 只从完整显式 synthetic 配置创建全新空 `root/data/marker`，或对既有候选根做双轮只读、脱敏核验；S14 为精确空库提供一次性成人管理员、四类获批合成法律元数据和不可变完成回执；S15 把 S12/S13/S14、当前已提交 36 文件/10 迁移、配置、物理根和只读 pristine SQLite/receipt 绑定成短时 machine subject，再把 19 项外部声明包装成未认证信封。S16 只用环境摘要钉住的独立公开策略验证 Ed25519 gate/checkpoint/approval/grant 签名束，并重跑 S15 当前状态；它不认证策略权威、身份或证据正文，不提供可信时间、单调吊销或原子 grant 消费，始终不授予部署或儿童使用。最终验证结果以最新交接为准，详见 [阶段 1 实施清单](docs/plans/阶段1-现有能力审计与首批实施清单-20260823.md)。
- 生产迁移已增加“旧库一致性快照 + 清单校验 + 无清单拒绝迁移”门禁；但正式法律文本、PIPIA、存量数据整改和 AppGallery 正式上架均未完成，所有儿童生产功能继续默认关闭。
- S0～S12 及 Git 迁移收尾已安全存在于专用远端；S13～S16 已在本地主题分支完成，尚未推送。远端落盘或本地完成都不代表已经部署、联网、上架或开放儿童功能。
...
- [仓库工作规范](AGENTS.md)
- [最新开发交接](docs/handoff/Codex-糖罐积分阶段1-S16外部签名束离线验证交接-20260828.md)
