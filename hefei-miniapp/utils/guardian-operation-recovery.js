// 仅持久化授权写入的非秘密恢复句柄；不保存密码、reauth、儿童资料或响应正文。
var STORAGE_KEY = 'hefei_guardian_consent_recovery_v1';
var MAX_RECORDS = 32;
var OPERATIONS = { 'child-enrollment': true, 'child-consent': true };
...
function createRecoveryStore(options) {
  ...
  if (!storage || typeof getUser !== 'function') throw new TypeError('recovery storage is required');

  function records() {
    if (typeof storage.getStorageSync !== 'function') {
      throw new Error('guardian recovery storage is unavailable');
    }
    try {
      ...
      if (!Array.isArray(parsed) || parsed.length > MAX_RECORDS
        ...
        throw new Error('guardian recovery storage is corrupt');
      }
      return parsed.slice();
    } catch (error) {
      throw new Error('guardian recovery storage cannot be read');
    }
  }
  ...
  function save(values) {
    ...
    if (values.length > MAX_RECORDS) throw new Error('guardian recovery storage is full');
  ...
  function begin(operation, idempotencyKey) {
    var user = getUser();
    if (!validUser(user) || !OPERATIONS[operation]
        || typeof idempotencyKey !== 'string' || !IDEMPOTENCY_KEY.test(idempotencyKey)) {
      throw new TypeError('guardian operation recovery marker is invalid');
    }
    var next = records().filter(function(item) {
      return item.actorId !== user.id || item.familyId !== user.familyId;
    });
    if (next.length >= MAX_RECORDS) throw new Error('guardian recovery storage is full');
    next.push({
      version: 1,
      actorId: user.id,
    ...
    save(next);
    var saved = current();
    if (!saved || saved.operation !== operation || saved.idempotencyKey !== idempotencyKey) {
      throw new Error('guardian recovery marker verification failed');
    }
    return saved;
  }
