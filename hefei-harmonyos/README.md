...
糖罐积分的原生 HarmonyOS 儿童客户端。当前 `0.2.0 (20000)` 已推进到阶段 1 / S20c：在设备配对、设备会话、本人积分摘要/流水、当前可申报规则、文字积分申报和“我的申请”、受控临时 synthetic profile 与纯本地“设置与数据安全”说明之上，增加当前已加载申请的手动单条只读详情，并为申请列表首次/全量刷新与分页追加建立单调、唯一和 fail-closed 边界。它不是完整薄 MVP，也未连接任何外部生产或非生产业务服务。
...
- transport 只允许设备申领/完成配对、Refresh challenge/轮换、本人摘要/流水、设备 reward rules，以及本人积分申请创建/列表/单条详情的固定方法与路径；S20a 的 resubmit/cancel 历史结果只读对账仍只是协议/client 基座，不被消费层调用
- 不提供家庭、儿童、设备或会话的客户端身份选择；身份只从服务端设备凭据推导
- 不申请 `MANAGE_SCREEN_TIME_GUARD` ACL，不启用照片、Push、广告、支付、第三方统计或用机兑换
- 不实现申请补充、取消或重新提交：S20a 已补齐单条详情与既有写操作历史结果对账的技术基座，S20b 只消费单条 GET，S20c 只加固列表/详情只读快照；未决 description 的生产保留期限与监护人授权安全放弃策略获批前，禁止接入 mutation UI、持久 mutation coordinator，或用结果猜测替代精确恢复
...
- 申请列表页内及跨页要求 canonical ID、来源可见 `clientRequestId` 唯一，并与服务端 `(submittedAt, id) DESC` 顺序一致；拒绝空页续游标、同游标不前进和多步游标循环，任何坏页都在应用前原子失败
- 全量刷新逐项约束旧到新 revision/时间单调性，并拒绝跨 ID 重映射已有来源 `clientRequestId`；协议错误保留此前已验证的只读申请快照，但清空规则和申请/规则游标，阻断继续翻页或沿用旧规则创建申请
- 用户只能从当前内存申请列表手动读取唯一 canonical ID 的单条详情；selected detail 原位单调更新且只存在于内存，收起、进入后台或收到明确撤权/授权错误会清除详情，界面不展示内部申请 ID、`clientRequestId` 或 transaction ID
...
- S10 静态门固定安全壳导出面、`main_pages.json` 中唯一可达的 `Index` 页面、`Index.ets` 内唯一 Entry、受信 Builder/生命周期签名以及允许的本地数据和事件，并用 decoy、re-export、非白名单模板插值、动态身份、存储/网络副作用和入口绕过夹具防回归；S20b/S20c 静态词法门另扫描 HarmonyOS 消费层 `.ets`/`.ts` 的直接符号与字面路径，禁止消费 point-request operation reconcile 或 resubmit/cancel mutation，并固定 pending intent 时的分页 UI 禁用与列表协议冻结。它们都是 defense-in-depth，不替代人工安全审查
...
S20c 最终结果为根级 `npm test` 362/362、point-request 23/23、HarmonyOS synthetic generator 7/7、静态安全门 12/12、`npm run check` 与 committed preflight 通过，以及来自最终 committed HarmonyOS 源的临时 unsigned 工程 BUILD SUCCESSFUL、Hypium 66/66。本机 DevEco 随附的 CodeLinter CLI 包装入口 `plugins/codelinter/run/index.js` 已能正确识别已安装 SDK，结果为 0 error、0 warning、1 suggestion；唯一建议是现有 1024×1024 启动图标宜导出为不超过 256×256 的获批品牌版本，本轮没有用通用模板或生成式重绘替换品牌图标。多路最终复核无 P0/P1。以上结果只证明本地安全切片、合成回归、静态门和 unsigned 编译，不代表正式法律文本、HUKS/AssetStore 真机行为、外部联网端到端链路、签名包或 AppGallery 包已经验证。
...
- 在未决文字的生产保留期限及监护人授权安全放弃流程获批并实现持久 mutation coordinator 前，继续关闭申请补充、取消与重新提交；S20a/S20b/S20c 的只读协议、界面与列表 fence 不能替代这些产品和合规决定
