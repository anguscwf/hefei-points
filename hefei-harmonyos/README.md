...
糖罐积分的原生 HarmonyOS 儿童客户端。当前 `0.2.0 (20000)` 已推进到阶段 1 / S20b：在设备配对、设备会话、本人积分摘要/流水、当前可申报规则、文字积分申报和“我的申请”、受控临时 synthetic profile 与纯本地“设置与数据安全”说明之上，增加当前已加载申请的手动单条只读详情。它不是完整薄 MVP，也未连接任何外部生产或非生产业务服务。
...
- 不实现申请补充、取消或重新提交：S20a 已补齐单条详情与既有写操作历史结果对账的技术基座，S20b 只消费单条 GET；未决 description 的生产保留期限与监护人授权安全放弃策略获批前，禁止接入 mutation UI、持久 mutation coordinator，或用结果猜测替代精确恢复
...
- S10 静态门固定安全壳导出面、`main_pages.json` 中唯一可达的 `Index` 页面、`Index.ets` 内唯一 Entry、受信 Builder/生命周期签名以及允许的本地数据和事件，并用 decoy、re-export、非白名单模板插值、动态身份、存储/网络副作用和入口绕过夹具防回归；S20b 静态词法门另扫描 HarmonyOS 消费层 `.ets`/`.ts` 的直接符号与字面路径，禁止消费 point-request operation reconcile 或 resubmit/cancel mutation。它们都是 defense-in-depth，不替代人工安全审查
...
- 在未决文字的生产保留期限及监护人授权安全放弃流程获批并实现持久 mutation coordinator 前，继续关闭申请补充、取消与重新提交；S20a/S20b 的只读协议与界面不能替代这些产品和合规决定
