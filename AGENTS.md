...
- `origin` 指向专用仓 `git@github.com:anguscwf/hefei-points.git`（2026-08-27 迁移完成）。迁移前曾共用的 `git@github.com:anguscwf/projects.git` 已于同日清除全部 18 条糖罐分支引用；其 `master` 专属于父 monorepo，与本仓历史无共同祖先。本仓禁止创建、检出、合并或推送 `master`，也不得把任何父仓 `master` 合入本仓分支。
- 本仓长期稳定分支为 `main`，阶段 1 汇总分支为 `codex/stage1`；普通 Codex 主题分支必须使用 `codex/<topic>-YYYYMMDD`。禁止使用含糊的 `master`、`dev`、`tmp` 或无前缀远端分支。
- 每次获准 push 前必须先执行 `git fetch --prune origin`，再用 `git ls-remote --heads origin` 核对目标 ref；专用仓不得出现 `master`。
...
- 本地必须设置 `push.default=nothing`，使遗漏显式 refspec 的 push 直接失败。2026-08-27 已完成向专用 `hefei-points` 远端的迁移（决策、默认分支与 18 条 ref OID 已逐条核对一致），并在双仓逐项同 SHA 的硬门通过后清除旧大仓对应引用；如未来再调整 origin，必须先记录决策、再核对默认分支和全部 ref OID，最后修改 `origin`。
- 2026-08-27 本地独立仓因 Git 2.55 `files` 引用后端反复丢失 loose refs，已在完整备份后迁移为 `reftable` 并重建 18 个本地分支。工具若不支持 `reftable` 必须停止操作并改用兼容 Git，禁止手写、清空或替换 `.git` 引用文件。
